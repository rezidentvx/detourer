#pragma once
#include "detourerconfig.h"
#include <windows.h>
#include <Psapi.h> // GetModuleInformation
#include <libloaderapi.h> // GetModuleFileName
#include <string>
#include <vector>
#include <array>
//#include <winerror.h>
//#include <WinUser.h>

// Stringizing
#define _CAT(A, B) A ## B
#define CAT(A, B) _CAT(A, B)
#define _STRCAT(A, B) A B
#define STRCAT(A, B) _STRCAT(A, B)
#define _STR(s) #s
#define STR(s) _STR(s)
#define _WSTR(s) //CAT(L, STR(s))
#define WSTR(s) L ## #s //_WSTR(s)

// Select overloaded macro
#define SELECT( NAME, NUM ) CAT( NAME ## _, NUM )
#define COMPOSE( NAME, ARGS ) NAME ARGS
#define GET_COUNT( _1, _2, _3, _4, _5, _6, COUNT, ... ) COUNT
//#define EXPAND() ,,,,,, // 6 commas (or 7 empty tokens)
#define VA_SIZE( ... ) GET_COUNT( __VA_ARGS__, 6, 5, 4, 3, 2, 1 )
#define VA_SELECT( NAME, ... ) SELECT( NAME, VA_SIZE(__VA_ARGS__) )(__VA_ARGS__)

#define TRY(...) VA_SELECT(TRY, __VA_ARGS__)

// If an attempt fails...
#if ASSERT_SUCCESS_LEVEL == 2 // Generate an exception in the caller
#define TRY_ASSERT_SUCCESS(...) throw
#define TRY_1(OPERATION) TRY1(OPERATION, WSTR(OPERATION));

#elif ASSERT_SUCCESS_LEVEL == 1 // Make the caller return false (DANGEROUS if bad return value) by catching a throw to return false
#define TRY_ASSERT_SUCCESS(...) throw
#define TRY_1(OPERATION) try { TRY1(OPERATION, WSTR(OPERATION)); } catch (...) { return false; }

#else // Act like it never happened & carry on
#define TRY_ASSERT_SUCCESS(...) (void)0
#endif

int TRY1(int retval, LPCWSTR operation);
#define TRY_2(OPERATION, FUNC) TRY_1((OPERATION##FUNC())) //TRY_1(CAT(OPERATION,CAT(FUNC, STR(())))

//inline BOOL Test(int val) { // Shouldn't #define Test(val) (val) == NO_ERROR Because macros are bad
//    return val == NO_ERROR;
//}

void OUT_MB(LPCWSTR message, LPCWSTR header);

#ifdef _DEBUG
void OUT_DEBUG(LPCWSTR message, LPCWSTR header);
void OUT_FAIL(LPCWSTR operation, LPCWSTR location = nullptr);
//#define OUT_FAIL(OPERATION) OUT_DEBUG(_STRCAT(L"Failed to ", WSTR(OPERATION)), L"Detourer Failure")
// OUT_FAIL_1 param is legacy. _2 is experimental. Godspeed.
//#define OUT_FAIL(...) VA_SELECT(OUT_FAIL, __VA_ARGS__)
//#define OUT_FAIL_1(OPERATION) OUT_DEBUG(_STRCAT(L"Failed to ", WSTR(OPERATION)), L"Detourer Failure")
//#define OUT_FAIL_2(OPERATION, LOCATION) OUT_DEBUG(_STRCAT(L"Failed to ", WSTR(OPERATION)), _STRCAT(L"FAIL: ", WSTR(LOCATION)))
#else
#define OUT_DEBUG(...) (void)0
#define OUT_FAIL(...) (void)0
#endif

// Somewhere in this stack, Debug builds are unable to resolve the module name from pointer
HMODULE GetModuleAtAddress(LPVOID addr);
std::string GetModuleNameStr(LPVOID addr);
std::string GetModuleNameStr(HMODULE hmod);